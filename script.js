// jsPDF & html2canvas (~600KB gabungan) hanya dipakai jauh di stage sertifikat/voucher —
// dimuat on-demand saat pertama dibutuhkan, bukan di awal, supaya taman tampil lebih cepat.
...
window._ensureJsPDF = function(){ return window._loadScriptOnce('https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js'); };
window._ensureHtml2Canvas = function(){ return window._loadScriptOnce('https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js'); };
// Kasih tahu pengguna kalau tombol lagi menunggu library ~600KB diunduh (baru terjadi sekali per stage per sesi)
window._withLoadingLabel = function(btn, label, promise){
  if(!btn) return promise;
  var prev = btn.innerHTML, prevDisabled = btn.disabled;
  btn.innerHTML = label;
  btn.disabled = true;
  return promise.then(function(v){ btn.innerHTML = prev; btn.disabled = prevDisabled; return v; },
                       function(e){ btn.innerHTML = prev; btn.disabled = prevDisabled; throw e; });
};
...
window.pzState={locked:0,dragging:null,baseLeft:0,baseTop:0,downX:0,downY:0};
(function(){
  ...
  function onPD(e){
    ...
    if(piece.dataset.inTray==='1'){
      ...
      window.pzState.baseLeft=initL;window.pzState.baseTop=initT;
    } else {
      piece.style.zIndex='999';piece.style.cursor='grabbing';
      window.pzState.baseLeft=parseFloat(piece.style.left||0);window.pzState.baseTop=parseFloat(piece.style.top||0);
    }
    window.pzState.downX=e.clientX;window.pzState.downY=e.clientY;
  ...
  function onPM(e){
    ...
    e.preventDefault();
    // Selama drag: cuma transform (compositor-only), left/top TIDAK disentuh -> tidak ada reflow tiap gerak mouse
    var dx=e.clientX-window.pzState.downX,dy=e.clientY-window.pzState.downY;
  ...
  function onPU(e){
    ...
    // Selesai drag: konversi transform delta jadi left/top final (hasilnya identik dengan sistem lama)
    var dxF=e.clientX-window.pzState.downX,dyF=e.clientY-window.pzState.downY;
    piece.style.transform='';
    piece.style.left=(window.pzState.baseLeft+dxF)+'px';
    piece.style.top=(window.pzState.baseTop+dyF)+'px';
...
(function(){
  ...
  var W=280,H=280;
  // Retina/high-DPI: backing store diskalakan sesuai devicePixelRatio (di-cap 2x agar tidak boros),
  // ukuran tampilan CSS tetap 280x280 — supaya tajam di layar laptop resolusi tinggi.
  var dprVG=Math.min(window.devicePixelRatio||1,2);
  C.width=W*dprVG;C.height=H*dprVG;
  C.style.width=W+'px';C.style.height=H+'px';
  ctx.setTransform(dprVG,0,0,dprVG,0,0);
  ...
  function drawFlower(){
    ...
    var ps=clamp((totalP()-0.75)/0.2,0,1);
    var sh=stemHeight(),bx=W/2,by=H-56;
    var fx=bx,fy=by-sh-2;

    // Cakram tengah digambar LEBIH DULU. Sebelumnya cakram ini digambar
    // setelah kelopak dan ukurannya sudah penuh sejak awal mekar (br besar
    // duluan sebelum kelopak sempat terlihat) — jadi kelopak yang baru mulai
    // tumbuh (pr masih kecil) selalu tertutup total oleh cakram di atasnya,
    // dan yang tampak cuma bulatan polos di ujung batang.
    var br=bs*(ps>0.5?11:8+bs*6);
...
const BG_MOODS = {
  forest: 'radial-gradient(ellipse at 20% 30%,rgba(72,152,192,.34) 0%,transparent 65%),radial-gradient(ellipse at 75% 70%,rgba(58,110,150,.20) 0%,transparent 65%),linear-gradient(160deg,#0A1830 0%,#123152 55%,#1C4A73 100%)',
  rose:   'radial-gradient(ellipse at 50% 30%,rgba(108,132,222,.38) 0%,transparent 70%),radial-gradient(ellipse at 20% 80%,rgba(150,182,235,.20) 0%,transparent 65%),linear-gradient(160deg,#0A1830 0%,#123152 55%,#1C4A73 100%)',
  moon:   'radial-gradient(ellipse at 60% 20%,rgba(143,203,234,.46) 0%,transparent 70%),radial-gradient(ellipse at 30% 85%,rgba(180,220,245,.16) 0%,transparent 55%),linear-gradient(160deg,#0A1830 0%,#123152 55%,#1C4A73 100%)',
  gold:   'radial-gradient(ellipse at 50% 35%,rgba(64,120,200,.42) 0%,transparent 70%),radial-gradient(ellipse at 80% 75%,rgba(100,160,220,.24) 0%,transparent 65%),linear-gradient(160deg,#0A1830 0%,#123152 55%,#1C4A73 100%)',
};
...
function showAlert(msg,icon){
  ...
  try{sfx('err');}catch(e){}
  if(_caTimer)clearTimeout(_caTimer);
  _caTimer=setTimeout(closeAlert,3200);
}
function closeAlert(){
  const m=document.getElementById('ca-modal');
  if(!m)return;
  m.style.opacity='0';
  m.style.transform='translateX(-50%) translateY(16px)';
  m.style.pointerEvents='none';
  if(_caTimer){clearTimeout(_caTimer);_caTimer=null;}
}
document.addEventListener('keydown',function(e){if(e.key==='Escape'){const m=document.getElementById('ca-modal');if(m&&m.style.opacity==='1')closeAlert();}});
...
function go(f,t){
  // Pengaman: kalau meninggalkan stage tiup lilin tanpa sempat meniup, musik jangan nyangkut diam selamanya
  if(f==='s10'&&t!=='s10'&&typeof resumeAfterMic==='function')resumeAfterMic();
...
let pbOptions={mode:'strip', filter:'natural', frame:'blankBday'};
...
function pbToggleOptionsPanel(forceState){
  if(typeof forceState!=='boolean')sfx(); // cuma bunyi kalau diklik manual, bukan saat dipicu otomatis oleh sistem
...
function initPhotobooth(){
  ...
  pbUpdateLivePreview();
  pbUpdateOptionsSummary();
  pbToggleOptionsPanel(false); // langsung terbuka — pilihan template tampil sejak awal
  pbStartIsoJitter();
...
// PARALLAX — target di-cache per pergantian stage, bukan di-query ulang tiap frame,
// dan transform cuma ditulis kalau posisinya benar-benar berubah (hemat layout/paint saat kursor diam)
...
let _ploopTargets=[];
function refreshPloopTargets(){
  _ploopTargets=Array.prototype.slice.call(document.querySelectorAll('.screen.active .gc,.screen.active .lscroll,.screen.active .rbg,.screen.active .ccard,.screen.active .capform,.screen.active .dgrid,.screen.active .tgrid2,.screen.active .phwrap,.screen.active .cstage,.screen.active .grow'));
}
refreshPloopTargets();
(function ploop(){
  const pmx=mx,pmy=my;
  mx+=(tx-mx)*.05;my+=(ty-my)*.05;
  if(Math.abs(mx-pmx)>.02||Math.abs(my-pmy)>.02){
    const t=`translate3d(${mx}px,${my}px,0)`;
    for(let i=0;i<_ploopTargets.length;i++)_ploopTargets[i].style.transform=t;
  }
...
// HINTS
function showHint(s){sfx('tr');if(s===1){document.getElementById('h1').textContent="Petunjuk: 2⁵ = 32, dikurangi 12 = Ytta.";document.getElementById('h1').style.color='var(--sage-lt)';}else{document.getElementById('h2').textContent="Petunjuk: Gampang inimah buat anak akutansi.";document.getElementById('h2').style.color='var(--rose-lt)';}}

// VERIFY
function verify1(){const a=document.getElementById('ca').value.trim();if(a==='20'){sfx('ok');miniConfetti();go('s2','s2b');}else{sfx('err');document.getElementById('ca').value='';showAlert('Masa gitu aja salah! 🌸');}}
function verify2(){const a=document.getElementById('cb').value.trim();if(a==='2007'){sfx('ok');miniConfetti();startCD();go('s2b','scd');}else{sfx('err');document.getElementById('cb').value='';showAlert('Salah kocak 🗿');}}
...
function startCD(){
  ...
  function tickVal(id,newVal,key){
    ...
    if(!el||prevCD[key]===newVal)return;
    el.textContent=newVal;
    prevCD[key]=newVal;
    var block=el.closest('.tblock');
    el.classList.remove('ticking');void el.offsetWidth;el.classList.add('ticking');
    if(block){
      block.classList.remove('ticking');void block.offsetWidth;block.classList.add('ticking');
      setTimeout(function(){block.classList.remove('ticking');},450);
    }
  }
  cdInt=setInterval(()=>{
    const n=new Date(),yr=n.getFullYear()-BIRTH.getFullYear();
    let ann=new Date(BIRTH);ann.setFullYear(n.getFullYear());
    if(n<ann)ann.setFullYear(n.getFullYear()-1);
    const d=Math.floor((n-ann)/864e5);
    tickVal('cdy',String(yr).padStart(2,'0'),'y');
    tickVal('cdd',String(d).padStart(2,'0'),'d');
    tickVal('cdh',String(n.getHours()).padStart(2,'0'),'h');
...
// ===== Peta Takdir — gelar Stage 8 kini dihitung otomatis dari simbol Stage 6, bukan dipilih ulang =====
const DEST_INFO={
  'The Moonlit Gardener':{desc:'Jiwa penjaga taman bulan yang merawat impian di bawah cahaya rembulan.',icon:'<svg viewBox="0 0 40 40" width="36" height="36" fill="none"><path d="M28 20a12 12 0 1 1-12-12 9 9 0 0 0 12 12z" fill="#8FCBEA" opacity=".85"/><circle cx="26" cy="12" r="1.5" fill="#F6CB7A" opacity=".6"/><circle cx="30" cy="17" r="1" fill="#F6CB7A" opacity=".5"/></svg>'},
  'The Botanical Dreamer':{desc:'Pribadi pemimpi yang menemukan keajaiban dalam setiap helai daun dan musim.',icon:'<svg viewBox="0 0 40 40" width="36" height="36" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M20 34 C20 34 8 26 8 16 C8 10 14 6 20 10 C26 6 32 10 32 16 C32 26 20 34 20 34Z" fill="#7FAE6A" opacity=".8"/><line x1="20" y1="34" x2="20" y2="14" stroke="#B2CEA6" stroke-width="1.5" stroke-linecap="round"/><line x1="20" y1="26" x2="14" y2="20" stroke="#B2CEA6" stroke-width="1" stroke-linecap="round" opacity=".7"/><line x1="20" y1="22" x2="26" y2="18" stroke="#B2CEA6" stroke-width="1" stroke-linecap="round" opacity=".7"/></svg>'},
  'The Wildflower Sovereign':{desc:'Perempuan berdaulat, mekar liar dan indah, tak terbendung oleh badai apapun.',icon:'<svg viewBox="0 0 40 40" width="36" height="36" fill="none"><circle cx="20" cy="20" r="7" fill="#F2B441" opacity=".9"/><g><ellipse cx="20" cy="8" rx="4" ry="7" fill="#F6CB7A" opacity=".75"/><ellipse cx="20" cy="8" rx="4" ry="7" fill="#F6CB7A" opacity=".75" transform="rotate(45 20 20)"/><ellipse cx="20" cy="8" rx="4" ry="7" fill="#F6CB7A" opacity=".75" transform="rotate(90 20 20)"/><ellipse cx="20" cy="8" rx="4" ry="7" fill="#F6CB7A" opacity=".75" transform="rotate(135 20 20)"/><ellipse cx="20" cy="8" rx="4" ry="7" fill="#F6CB7A" opacity=".75" transform="rotate(180 20 20)"/><ellipse cx="20" cy="8" rx="4" ry="7" fill="#F6CB7A" opacity=".75" transform="rotate(225 20 20)"/><ellipse cx="20" cy="8" rx="4" ry="7" fill="#F6CB7A" opacity=".75" transform="rotate(270 20 20)"/><ellipse cx="20" cy="8" rx="4" ry="7" fill="#F6CB7A" opacity=".75" transform="rotate(315 20 20)"/></g><circle cx="20" cy="20" r="7" fill="#F2B441" opacity=".9"/></svg>'}
};
const ANIMAL_DEST={'Kupu-Kupu (Metamorfosis)':'The Botanical Dreamer','Cendrawasih (Keanggunan)':'The Wildflower Sovereign','Rusa (Intuisi)':'The Moonlit Gardener','Merpati (Kedamaian)':'The Botanical Dreamer'};
const FLOWER_DEST={'Kamomil (Resiliensi)':'The Wildflower Sovereign','Teratai (Kemurnian)':'The Moonlit Gardener','Magnolia (Martabat)':'The Wildflower Sovereign','Lavendel (Ketenangan)':'The Moonlit Gardener'};
function computeDestiny(f,a){
  const scores={'The Moonlit Gardener':0,'The Botanical Dreamer':0,'The Wildflower Sovereign':0};
  if(ANIMAL_DEST[a])scores[ANIMAL_DEST[a]]+=2;
  if(FLOWER_DEST[f])scores[FLOWER_DEST[f]]+=1;
  let best='The Wildflower Sovereign',bv=-1;
  Object.keys(scores).forEach(k=>{if(scores[k]>bv){bv=scores[k];best=k;}});
  return best;
}
function confirmPhilo(){
  sfx('ok');
  document.getElementById('cflval').textContent='Esensi Bunga: '+selF;
  document.getElementById('canval').textContent='Esensi Hewan: '+selA;
  selD=computeDestiny(selF,selA);
  // selD tetap dihitung diam-diam di sini untuk sertifikat (cdyntitle/dynbadge) —
  // stage 8 sekarang jadi "Penyegelan Motto", bukan lagi tempat menampilkan gelar takdir ini.
  try{document.getElementById('cdyntitle').textContent='[ '+selD+' ]';}catch(e){}
  try{document.getElementById('dynbadge').textContent=selD;}catch(e){}
  go('sph','s3');
}

// CHOICE / DESTINY
let _mottoRevealed={ad:false,maiora:false,natus:false,sum:false};
function revealMottoWord(btn,word){
  if(!btn||btn.classList.contains('used'))return;
  btn.classList.add('used');
  sfx('ok');
  const target=document.querySelector('.motto-word[data-word="'+word+'"]');
  if(target){
    target.classList.add('revealed');
    const r=target.getBoundingClientRect();
    bloom(r.left+r.width/2,r.top+r.height/2,'#F2B441');
  }
  _mottoRevealed[word]=true;
  if(Object.keys(_mottoRevealed).every(k=>_mottoRevealed[k])){
    setTimeout(()=>{
      const tr=document.getElementById('mottoTranslation');
      if(tr)tr.style.opacity='1';
      const nb=document.getElementById('nmotto');
      if(nb)nb.style.display='inline-flex';
    },500);
  }
}
function pickReflect(el){
  if(!el)return;
  ...
  bloom(r.left+r.width/2,r.top+r.height/2,'#F2B441');
  const cl=document.getElementById('reflectClosing');
  if(cl)cl.style.opacity='1';
  const np=document.getElementById('npath');
...
function endCin(){sfx('tr');if(ptmr){clearTimeout(ptmr);ptmr=null;}if(panim){clearTimeout(panim);panim=null;}['cskip','pmain','pname'].forEach(id=>{const e=document.getElementById(id);if(e){e.style.opacity='0';if(e.tagName==='BUTTON')e.style.pointerEvents='none';}});setBG('rose');go('sci','spz');}

// S7, S8
function goS8(){sfx('tr');setBG('letter');go('srh','s8');trigLetter();}
function trigLetter(){
...
function downloadSuratPDF(btn){
  ...
  window._withLoadingLabel(btn,'✦ Menyiapkan...',window._ensureJsPDF()).catch(()=>{}).then(function(){
...
function execBlow(){
  ...
  // Musik TETAP diam sepanjang lilin padam + overlay gelap membangun di stage ini —
  // fade-in dipicu otomatis oleh pengaman di go() tepat saat go('s10','s11') dipanggil,
  // supaya jeda hening itu terasa seperti bagian dari momennya, bukan terpotong lebih awal.
  setTimeout(()=>{
    ov.remove();
    setBG('gold');
    go('s10','s11');
...
function initMic(){if(micOn||!navigator.mediaDevices)return;setBG('cake');navigator.mediaDevices.getUserMedia({audio:{echoCancellation:false,noiseSuppression:false,autoGainControl:false},video:false}).then(str=>{if(micStr)micStr.getTracks().forEach(t=>t.stop());micStr=str;const mc=new(window.AudioContext||window.webkitAudioContext)(),ms=mc.createMediaStreamSource(str);micAnl=mc.createAnalyser();micAnl.fftSize=1024;const hp=mc.createBiquadFilter();hp.type='highpass';hp.frequency.setValueAtTime(100,mc.currentTime);const lp=mc.createBiquadFilter();lp.type='lowpass';lp.frequency.setValueAtTime(800,mc.currentTime);const gn=mc.createGain();gn.gain.setValueAtTime(3.5,mc.currentTime);ms.connect(hp);hp.connect(lp);lp.connect(gn);gn.connect(micAnl);micData=new Uint8Array(micAnl.frequencyBinCount);micOn=true;if(typeof pauseForMic==='function')pauseForMic();
    ...
    window._micMuteWatch=setInterval(function(){
      ...
      if(am&&!am.paused&&!am._fadeIv&&am.volume>0.101){am._muteForMic=true;am.volume=0.1;}
    },250);
...
function claimVoucher(type,btn){
  sfx('ok');
  const data = {
    makan: {title:'VOUCHER MAKAN', icon:'🍽️', desc:'Berlaku untuk satu kali traktiran makan, Rizqi yang bayar.', code:'MAKAN-NAFFA20', file:'Voucher_Makan_Naffa.png'},
    nonton:{title:'VOUCHER NONTON', icon:'🎬', desc:'Berlaku untuk satu kali nonton film pilihanmu, Rizqi yang traktir tiketnya.', code:'NONTON-NAFFA20', file:'Voucher_Nonton_Naffa.png'},
    main:  {title:'VOUCHER MAIN',   icon:'🎮', desc:'Berlaku untuk satu kali sesi main apapun itu, Rizqi temenin sekaligus bayarin.', code:'MAIN-NAFFA20', file:'Voucher_Main_Naffa.png'}
  }[type];
  if(!data) return;
  if(typeof pbNotify==='function')pbNotify('Klaim Voucher', data.title);
  ...
  window._withLoadingLabel(btn,'✦ Menyiapkan...',window._ensureHtml2Canvas()).catch(()=>{}).then(function(){
...
// ===== PESAN RAHASIA — disembunyikan di tanda tangan "Rizqi Maulana" di sertifikat =====
function openSecretLetter(){
  const modal=document.getElementById('secret-letter-modal');
  if(!modal) return;
  modal.style.display='flex';
  try{sfx('win');}catch(e){}
  if(!localStorage.getItem('taman_secret_letter_seen')){
    localStorage.setItem('taman_secret_letter_seen','1');
    try{miniConfetti();}catch(e){}
    if(typeof pbNotify==='function') pbNotify('Naffa menemukan pesan rahasia di sertifikat ✦');
  }
}
...
document.addEventListener('keydown', function(e){
  if(e.key==='Escape'){
    const modal=document.getElementById('secret-letter-modal');
    if(modal && modal.style.display==='flex') closeSecretLetter();
  }
});

function toggleSaveMenu(){
  const dd=document.getElementById('save-menu-dropdown');
  if(!dd)return;
...
document.addEventListener('click', function(e){
  const wrap=document.querySelector('.save-menu-wrap');
  if(wrap && !wrap.contains(e.target)) hideSaveMenu();
});
function savePNG(btn){
  sfx('ok');
  const z=document.getElementById('pzone');
  if(!z){showAlert('Sertifikat belum siap.');return;}
  window._withLoadingLabel(btn,'✦ Menyiapkan...',window._ensureHtml2Canvas()).catch(()=>{}).then(function(){
    if(typeof html2canvas!=='undefined'){
      html2canvas(z,{backgroundColor:'#0C1626',scale:2,useCORS:false,logging:false}).then(cv=>{const a=document.createElement('a');a.download='Sertifikat_Naffa_20th.png';a.href=cv.toDataURL('image/png');a.click();}).catch(()=>window.print());
    } else window.print();
  });
}
...
function resetAll(){
  ...
  updNP(0);syncTrackButtons();openedG=0;selF='';selA='';selD='The Wildflower Sovereign';selAffirm='';
  // Reset stage motto (s3) — biar bisa disusun ulang saat taman direplay
  try{_mottoRevealed={ad:false,maiora:false,natus:false,sum:false};}catch(e){}
  try{document.querySelectorAll('.motto-word').forEach(function(w){w.classList.remove('revealed');});}catch(e){}
  try{document.querySelectorAll('.motto-tile').forEach(function(t){t.classList.remove('used');});}catch(e){}
  try{document.getElementById('mottoTranslation').style.opacity='0';}catch(e){}
  try{document.getElementById('nmotto').style.display='none';}catch(e){}
  // Reset stage refleksi (s3b)
  try{document.querySelectorAll('.reflect-card').forEach(function(c){c.classList.remove('selected');});}catch(e){}
  try{document.getElementById('reflectClosing').style.opacity='0';}catch(e){}
...
function goS10() {
  ...
  setBG('moon');
  go('s9b','s10');
  if(typeof pauseForMic==='function')pauseForMic();
  setTimeout(function(){
    initCakeCanvas();
    var gate=document.getElementById('mic-gate');
...
function devSkip(id){
  ...
  if(id==='scd')startCD();
  if(id==='s10'){if(typeof pauseForMic==='function')pauseForMic();setTimeout(function(){initCakeCanvas();var gate=document.getElementById('mic-gate');if(gate)gate.style.display='flex';},900);}
  if(id==='sphoto'){setTimeout(initPhotobooth,300);}
  if(id==='s11')initS11MagneticNum();
}
...
let s11AnimId = null;
let s11Particles = [];
let s11W = 0, s11H = 0; // dimensi logis (CSS px) — dipakai di semua perhitungan partikel

function initS11Canvas() {
  const cv = document.getElementById('s11-canvas');
  if (!cv) return;
  // Retina/high-DPI: backing store diskalakan sesuai devicePixelRatio (di-cap 2x) —
  // supaya confetti di layar "20" besar ini tajam di laptop resolusi tinggi.
  const dpr = Math.min(window.devicePixelRatio || 1, 2);
  s11W = window.innerWidth; s11H = window.innerHeight;
  cv.width = s11W * dpr; cv.height = s11H * dpr;
  cv.style.width = s11W + 'px'; cv.style.height = s11H + 'px';
...
function s11Burst(color) {  for (let i = 0; i < 40; i++) {
    const cv = document.getElementById('s11-canvas');
    if (!cv) return;
    s11Particles.push({
      x: s11W / 2 + (Math.random() - 0.5) * s11W * 0.6,
      y: s11H / 2 + (Math.random() - 0.5) * s11H * 0.4,
...
// Magnetic-hover pada angka "20" — motif sama dengan huruf N di gerbang, dipasang sekali
let _s11MagInit = false;
function initS11MagneticNum(){
  if(_s11MagInit) return;
  const fine = window.matchMedia && window.matchMedia('(hover:hover) and (pointer:fine)').matches;
  const reduced = window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  if(!fine || reduced) return;
  const num = document.getElementById('s11-num');
  const scr = document.getElementById('s11');
  if(!num || !scr) return;
  _s11MagInit = true;
  scr.addEventListener('mousemove', function(e){
    ...
    if(dist < 160){
      ...
      num.style.transform = 'translate(' + (ex/160)*10*pull + 'px,' + (ey/160)*10*pull + 'px) scale(' + (1 + .06*pull) + ')';
    } else {
      num.style.transform = '';
    }
  });
  scr.addEventListener('mouseleave', function(){ num.style.transform = ''; });
}

// ===== RUANG HUJAN =====
// Cegah tombol "Biarkan Hujan Membawanya" dipicu berkali-kali selama animasi hujan berjalan.
let _rainSubmitted = false;
function startRain() {
  if (_rainSubmitted) return;
  ...
  // Kunci tombol & input supaya tidak bisa dikirim ulang selagi animasi berjalan.
  const submitBtn = document.getElementById('rh-submit-btn');
  if (submitBtn) { submitBtn.disabled = true; submitBtn.style.opacity = '.5'; submitBtn.style.pointerEvents = 'none'; submitBtn.style.cursor = 'default'; }
  const rhInputLock = document.getElementById('rh-input');
  if (rhInputLock) rhInputLock.disabled = true;
  ...
  const ctx = canvas.getContext('2d');
  // Retina/high-DPI: backing store diskalakan sesuai devicePixelRatio (di-cap 2x) —
  // supaya garis hujan tetap tajam di layar laptop resolusi tinggi, bukan buram.
  const dprRain = Math.min(window.devicePixelRatio || 1, 2);
  canvas.width = window.innerWidth * dprRain;
  canvas.height = window.innerHeight * dprRain;
  canvas.style.width = window.innerWidth + 'px';
  canvas.style.height = window.innerHeight + 'px';
  ctx.setTransform(dprRain, 0, 0, dprRain, 0, 0);
  const rainW = window.innerWidth, rainH = window.innerHeight;
  ...
  function drawRain() {
    ...
    if (frame % 18 === 0) {
      const rx = Math.random() * rainW;
      ctx.save();
      ctx.globalAlpha = 0.2;
      ctx.strokeStyle = 'rgba(170,210,240,.7)';
      ctx.lineWidth = 0.6;
      ctx.beginPath();
      ctx.ellipse(rx, rainH - 5, 6, 2, 0, 0, Math.PI * 2);
      ctx.stroke();
      ctx.restore();
    }
...
// ===== KOTAK KEJUTAN TERSEMBUNYI (langsung ke voucher, bukan surat lagi) =====

function openVoucherBox() {
  sfx('ok');
...
function pauseForMic(){
  if(!am||am.paused||am._muteForMic)return;
  am._muteForMic=true;
  if(am._fadeIv)clearInterval(am._fadeIv);
  var FLOOR=0.1; // redup ke volume rendah, bukan sampai bisu total — biar tidak terasa musiknya "hilang" tiba-tiba
  am._fadeIv=setInterval(function(){
    am.volume=Math.max(FLOOR,am.volume-.02); // langkah lebih kecil = turunnya lebih landai/perlahan
    if(am.volume<=FLOOR+0.001){am.volume=FLOOR;clearInterval(am._fadeIv);am._fadeIv=null;}
  },60);
}
function resumeAfterMic(){
  if(am&&am._muteForMic){
    am._muteForMic=false;
    if(am._fadeIv)clearInterval(am._fadeIv);
    // Langkah & jeda disamakan persis dengan pauseForMic (-.02 / 60ms) supaya naik-turunnya
    // terasa simetris — tidak ada sisi yang berasa lebih "kaget" dari sisi lainnya.
    am._fadeIv=setInterval(function(){am.volume=Math.min(1,am.volume+.02); if(am.volume>=1){am.volume=1;clearInterval(am._fadeIv);am._fadeIv=null;}},60);
  }
}
...
document.addEventListener('keydown',function(e){
  ...
  if(e.key==='Escape'){
    ...
    var saveDD=document.getElementById('save-menu-dropdown');
    if(saveDD&&saveDD.style.display==='block'){ hideSaveMenu(); return; }
  }
...
// Aksesibilitas keyboard: elemen interaktif non-native (div/span onclick) dibuatkan
// affordance keyboard otomatis — tabindex, role, dan Enter/Space memicu klik yang sama,
// supaya taman ini bisa dijelajahi penuh tanpa mouse.
(function enhanceKeyboardA11y(){
  document.querySelectorAll('[onclick]').forEach(function(el){
    var tag = el.tagName.toLowerCase();
    if(tag!=='div' && tag!=='span') return;
    if(el.id && /modal$/i.test(el.id)) return; // backdrop overlay, ditutup lewat Escape, bukan tombol
  ...
  document.addEventListener('keydown', function(e){
    ...
    ['voucher-modal','reply-modal'].forEach(function(id){
...
(function(){
  function getGreeting(){
    ...
    return 'Sudah malam atau sudah tahu';
  }
  var grSet=false;
  function setGreet(){
...
(function(){
  ...
  function startParticles(){
    ...
    var N = window.innerWidth < 600 ? 36 : 90;
    var pts = [];
    for(var i=0;i<N;i++){
      var depth = Math.random(); // 0=jauh(kecil,redup) 1=dekat(besar,terang) — depth-of-field
      pts.push({
    ...
    function loop(){
      ...
      for(var i=0;i<pts.length;i++){
        var p = pts[i];
        var a = (.18 + p.depth*.22) + Math.sin(now + p.ph)*.15;
        ctx.beginPath();
        ctx.arc(p.x, p.y, p.r, 0, Math.PI*2);
        ctx.fillStyle = p.c + Math.max(0,a) + ')';
        ctx.fill();
        p.y -= p.sp * (0.5 + p.depth*0.7);
...
// Parallax kartu + magnetic-hover huruf N — hanya untuk mouse presisi (desktop), hormati reduced-motion
  (function initGateCursorFX(){
    var fine = window.matchMedia && window.matchMedia('(hover:hover) and (pointer:fine)').matches;
    var reduced = window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    if(!fine || reduced) return;
    var card = document.querySelector('#lockgate .lk-card');
    var em = document.getElementById('lk-emletter');
    var gate = document.getElementById('lockgate');
    if(!card || !em || !gate) return;
    gate.addEventListener('mousemove', function(e){
      var r = card.getBoundingClientRect();
      var cx = r.left + r.width/2, cy = r.top + r.height/2;
      var dx = e.clientX - cx, dy = e.clientY - cy;
      card.style.transform = 'translate(' + Math.max(-7,Math.min(7,dx/40)) + 'px,' + Math.max(-7,Math.min(7,dy/40)) + 'px)';
      var emR = em.getBoundingClientRect();
      var ex = e.clientX - (emR.left+emR.width/2), ey = e.clientY - (emR.top+emR.height/2);
      var dist = Math.hypot(ex,ey);
      if(dist < 90){
        var pull = (90-dist)/90;
        em.style.transform = 'translate(' + (ex/90)*8*pull + 'px,' + (ey/90)*8*pull + 'px) scale(' + (1 + .1*pull) + ')';
      } else {
        em.style.transform = '';
      }
    });
